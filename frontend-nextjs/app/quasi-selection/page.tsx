...
function QuasiSelectionInner() {
  ...
  useEffect(() => {
    if (!sid) return;
    if (columns.length) return; // already loaded from context
    ...
    <div className="page-container">
      ...
      <div className="section">
        <div className="flex items-center justify-between flex-wrap gap-2">
          <h2 className="mb-0">🔍 Auto-Detect Column Types</h2>
          {detectResult && (
                    ...
                    {detectResult.details.map((row) => {
                      ...
                      return (
                        <tr key={row.column_name} className="hover:bg-gray-50 dark:hover:bg-slate-700/50 border-b border-gray-100 dark:border-slate-700">
                          ...
                          <td className="p-2">
                            <span className={`badge ${CLASS_BADGE[row.class] ?? ''}`}>{row.class}</span>
                          </td>
                          <td className="p-2">
                            <select
      ...
      {columns.length > 0 && (
        <div className="section">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mt-2">
              {columns.map(col => (
                <label key={col} className="flex items-center gap-2 p-2 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-600 rounded cursor-pointer hover:border-blue-500 dark:hover:border-blue-400 hover:bg-blue-50 dark:hover:bg-slate-700 transition-colors">
                  <input
                    type="checkbox"
                    checked={selectedQIs.has(col)}
                    onChange={() => toggleQI(col)}
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mt-2">
              {columns.map(col => (
                <label key={col} className="flex items-center gap-2 p-2 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-600 rounded cursor-pointer hover:border-blue-500 dark:hover:border-blue-400 hover:bg-blue-50 dark:hover:bg-slate-700 transition-colors">
                  <input
                    type="checkbox"
                    checked={selectedSens.has(col)}
                    onChange={() => toggleSens(col)}
                    className="accent-blue-600 dark:accent-blue-400 w-4 h-4"
                  />
                  <span className="text-sm text-slate-700 dark:text-slate-300">{col}</span>
                </label>
          ...
          <button
            ...
            disabled={saving || selectedQIs.size === 0}
          >
